...
SESSION_DIR.mkdir(exist_ok=True, mode=0o755)  # Явно указываем права доступа

# Временная директория для Pyrogram (чтобы избежать проблем с правами)
TEMP_DIR = Path(tempfile.gettempdir()) / "pyrogram_sessions"
...
def setup_termux():
    ...
    if 'com.termux' in os.environ.get('PREFIX', ''):
        ...
        try:
            ...
            print(f"⚠️ Проблема с правами доступа: {e}")
            print("🔄 Пробуем исправить...")
            os.system(f"chmod 755 {SESSION_DIR}")
            os.system(f"chmod 755 {TEMP_DIR}")
...
async def code_command(message: Message):
    ...
    try:
        # Создаем временную копию сессии для работы
        temp_session_path = TEMP_DIR / f"temp_{user_id}.session"
        
        # Копируем файл сессии во временную директорию
        import shutil
        shutil.copy2(session_data["session_path"], temp_session_path)
        
        # Подключение к аккаунту
        client = Client(
            name=str(temp_session_path.with_suffix('')),
            api_id=API_ID,
            api_hash=API_HASH,
...
async def handle_document(message: Message):
    ...
    try:
        ...
        # Скачиваем файл используя правильный метод
        ...
        temp_session_path = TEMP_DIR / f"temp_check_{user_id}.session"
        import shutil
        ...
        temp_client = Client(
            ...
            api_hash=API_HASH,
            workdir=str(TEMP_DIR),
            in_memory=True  # Используем in_memory режим чтобы не создавать файлы БД
        )
