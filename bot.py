...
from pyrogram.errors import FloodWait, ApiIdInvalid, AccessTokenInvalid
import re
import json
...
class Bot:
    ...
    def format_text(self, text: str, style: str = "normal") -> str:
        """Форматирование текста с эмодзи и рамками"""
        styles = {
            "header": f"{Emoji.SPARKLES} *{text}* {Emoji.SPARKLES}",
            "success": f"{Emoji.SUCCESS} *{text}*",
            "error": f"{Emoji.CROSS} *{text}*",
            "warning": f"{Emoji.WARNING} *{text}*",
            "info": f"{Emoji.INFO} {text}",
            "wait": f"{Emoji.WAIT} {text}",
            "channel": f"{Emoji.CHANNEL} *{text}*",
            "group": f"{Emoji.GROUP} *{text}*"
        }
    ...
    async def cmd_start(self, client: Client, message: Message):
        ...
        welcome_text = f"""
{Emoji.PARTY}{Emoji.CONFETTI}{Emoji.TADA}

*Добро пожаловать в Channel Creator Bot!* {Emoji.ROCKET}

{Emoji.HEART} Привет, *{first_name}*!

{Emoji.STAR} *О боте:* {Emoji.STAR}
Я помогу вам создавать каналы и группы в Telegram.
Просто настройте API и начинайте создавать!

{Emoji.SETTINGS} *Для начала работы:*
{Emoji.POINT} Настройте API через /settings
{Emoji.POINT} Создавайте каналы через /channel
{Emoji.POINT} Создавайте группы через /group

{Emoji.GEM} *Преимущества:*
{Emoji.CHECK} Массовое создание
{Emoji.CHECK} Автоматические задержки
{Emoji.CHECK} Красивые ссылки
{Emoji.CHECK} Удобный интерфейс
...
{Emoji.INFO} Используйте /help для подробной информации
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{Emoji.SETTINGS} Настройки", callback_data="settings"),
            ...
            [InlineKeyboardButton(f"{Emoji.STAR} Наш канал", url="https://t.me/your_channel"),
             InlineKeyboardButton(f"{Emoji.HEART} Поддержка", url="https://t.me/your_support")]
        ])
...
/help - Эта справка
    ...
    async def cmd_settings(self, client: Client, message: Message):
        ...
        has_api = 'api_id' in user_data and 'api_hash' in user_data
        
        if has_api:
            settings_text = f"""
{Emoji.SETTINGS} *Настройки API* {Emoji.SETTINGS}

{Emoji.LINE*20}

{Emoji.CHECK} *Текущие настройки:*
{Emoji.KEY} API ID: `{user_data['api_id']}`
{Emoji.LOCK} API Hash: `{user_data['api_hash'][:5]}...{user_data['api_hash'][-5:]}`

{Emoji.LINE*20}

{Emoji.INFO} Для изменения настроек введите новые данные.
            """
            
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton(f"{Emoji.EDIT} Изменить API", callback_data="change_api"),
                 InlineKeyboardButton(f"{Emoji.BACK} В меню", callback_data="main_menu")],
                [InlineKeyboardButton(f"{Emoji.CHANNEL} К каналам", callback_data="create_channel"),
                 InlineKeyboardButton(f"{Emoji.GROUP} К группам", callback_data="create_group")]
            ])
        else:
            settings_text = f"""
{Emoji.SETTINGS} *Настройка API* {Emoji.SETTINGS}

{Emoji.LINE*20}

{Emoji.WARNING} *API не настроен!*

Для работы бота необходимо настроить API:
...
(например: `1234567`)
    ...
    async def cmd_create_channel(self, client: Client, message: Message):
        ...
        if 'api_id' not in user_data or 'api_hash' not in user_data:
            error_text = f"""
{Emoji.CROSS} *Ошибка!*

{Emoji.WARNING} API не настроен!

Сначала настройте API через /settings
            """
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton(f"{Emoji.SETTINGS} Настроить API", callback_data="settings")]
            ])
...
{Emoji.ARROW} Используйте /menu для возврата в главное меню.
            """
        else:
            cancel_text = f"""
{Emoji.INFO} *Нет активных действий*

{Emoji.ARROW} Используйте /menu для просмотра команд.
            """
...
Попробуйте снова или отправьте /cancel для отмены.
            ...
            # Ждем API Hash
            api_hash = text
            api_id = self.user_states[user_id].get('api_id')
            
            # Сохраняем данные
            UserData.update(user_id, api_id=api_id, api_hash=api_hash)
            
            # Очищаем состояние
            del self.user_states[user_id]
            
            success_text = f"""
{Emoji.SUCCESS} *API успешно настроен!* {Emoji.SUCCESS}

{Emoji.LINE*20}

{Emoji.CHECK} API ID: `{api_id}`
{Emoji.CHECK} API Hash: `{api_hash[:5]}...{api_hash[-5:]}`
...
Попробуйте снова или отправьте /cancel для отмены.
    ...
    async def handle_callback(self, client: Client, callback_query: CallbackQuery):
        ...
        if data == "main_menu":
            ...
            self.user_states[user_id] = {'state': 'waiting_api_id'}
            change_text = f"""
{Emoji.SETTINGS} *Изменение API*

{Emoji.ARROW} *Введите новый API ID:*

(число, например: `1234567`)
            """
            await callback_query.message.edit_text(change_text, parse_mode="Markdown")
...
{Emoji.TIME} Ожидание: {CREATION_DELAY} сек
                ...
                try:
                    ...
                    try:
                        ...
                        invite_link = "🔒 приватный"
                    
                    results.append(f"{Emoji.CHECK} {channel_name} - {invite_link}")
                    created += 1
...
{Emoji.ARROW} Используйте /menu для продолжения
            """
