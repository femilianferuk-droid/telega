...
async def code_command(message: Message):
    ...
    try:
        # Используем временную директорию для работы с сессией
        temp_session_path = TEMP_DIR / f"temp_{user_id}_{Path(session_data['session_path']).name}"
        ...
        client = Client(
            ...
            in_memory=True  # Используем in_memory чтобы не создавать лишних файлов
        )
...
async def handle_document(message: Message):
    ...
    try:
        ...
        temp_session_path = TEMP_DIR / f"check_{session_filename}"
        import shutil
        ...
        phone_number = me.phone_number if me.phone_number else "Не указан"
